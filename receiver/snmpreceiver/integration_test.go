...
func TestIntegration(t *testing.T) {
	// remove nolint when https://github.com/jacktomcat/opentelemetry-collector-contrib/issues/24240 is resolved
	// nolint:staticcheck
	testCases := []struct {
		desc                    string
	...
	for _, testCase := range testCases {
		t.Run(testCase.desc, func(t *testing.T) {
			t.Skip("Flaky test, see https://github.com/jacktomcat/opentelemetry-collector-contrib/issues/21086")
			factory := NewFactory()
